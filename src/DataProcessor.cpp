...
/*!
@brief read from matrix format data
@note Each column is a sample and each row is a species abundance. Separate with commas or tabs.
*/
int DataProcessor::readMatData(ifstream &ifile){
...
/*!
@brief read from TSV
@note There are several types of TSV file: MGNify types and one FEAST type, this function aims at automatically detect tsv file's type and process it. When loading new tsv file with different type, you may edit this function to support it, or change tsv file's format to any MGNify/ FEAST type.
*/
SampleData* DataProcessor::readSingleTSV(ifstream &ifile,string name){
...
/*!
@brief read from multiple tsv files
@note Input is a file that holds multiple paths to tsv files. This function will read each tsv file by path
*/
int DataProcessor::readMultiTSV(ifstream &ifile){
    ...
    while(getline(ifile, nameBuffer)){
        ...
        try {
            sDBuf=readSingleTSV(TSVFile,buf);
            Data[buf]=sDBuf;
        } catch (...) {
            cout<<buf<<" Error"<<endl;
        }
...
/*!
@brief write package data
*/
int DataProcessor::writePackage(ofstream &ofile){
...
int DataProcessor::readPackage(ifstream &ifile){
...
int DataProcessor::writeTSV(ofstream &ofile){
...
int DataProcessor::printToTable(ofstream &ofile){
    unordered_map<int,int> nameMap;
    vector<int> names;
    int id=0;
    auto pIndex=p->getIndex();
...
int DataProcessor::readBinaryPackage(ifstream &ifile){
    SampleHead sHead;
    Infos info;
    char *buffer;
    SampleData* sData;
...
int DataProcessor::merge(DataProcessor &data){
...
int DataProcessor::writeBinaryPackage(ofstream &ofile){
    Infos info;
